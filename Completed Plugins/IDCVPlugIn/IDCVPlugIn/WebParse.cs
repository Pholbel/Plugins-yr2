...
namespace IDCVPlugIn
{
    public class WebParse
    {
        ...
        {
            Match exp = Regex.Match(response, "LicenseExpDate\" type=\"text\" value=\"(?<EXP>.*?)\" maxlength", RegOpt);
            Expiration = exp.Groups["EXP"].ToString();

            //Does not support sanctions
            Match GoodDiscipline = Regex.Match(response, "<span style=\"font-size: 120%\">None</span>", RegOpt);
        ...
        {
            MatchCollection fields = Regex.Matches(response, "<td align=\"right\" style=\"width: 25%.*?>(?<HEADER>.*?):",RegOpt);
            MatchCollection fields2 = Regex.Matches(response, "<input name=\"ctl00\\$CPH1\\$txt.*?type=\"text\".*?\"(?<Value>.*?)\"",RegOpt);
            ...
            if (fields.Count > 0)
            {
                ...
                {
                    string header =fields[idx].Groups["HEADER"].ToString().Trim();
